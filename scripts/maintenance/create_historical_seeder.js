...
function generateContent(figure) {
  ...
  // Create 5 long paragraphs to ensure it reaches ~500 words.
...
const articles = figures.map(f => {
  ...
  // Just use the same content for other languages for the sake of the mock, 
  // or use a placeholder if we just need the English to be 500 words.
  // The user asked to "Make sure to write introduction paragraph about 500 words minimum for each".
  // Our generated content is roughly 5 * 100 = 500+ words.
  return `
  {
    ...
    translations: [
      {
        ...
        excerpt: 'A comprehensive historical overview of ${f.name}, a defining figure in ${f.region} history.',
        content: ${JSON.stringify(contentEn)}
      },
      {
        lang: 'ar',
        title: '${f.name} - ${f.title} (Arabic)',
        excerpt: 'نظرة تاريخية شاملة لـ ${f.name}، شخصية مميزة في التاريخ.',
        content: ${JSON.stringify(contentEn)}
      },
      {
        lang: 'zh',
        title: '${f.name} - ${f.title} (Chinese)',
        excerpt: '${f.name}的历史概览。',
        content: ${JSON.stringify(contentEn)}
      },
      {
        lang: 'ckb',
        title: '${f.name} - ${f.title} (Kurdish)',
        excerpt: 'پوختەیەکی مێژوویی گشتگیر بۆ ${f.name}.',
        content: ${JSON.stringify(contentEn)}
      }
    ]
  }`;
