import { LucideIcon, Factory, Search, ShieldCheck, Ship } from 'lucide-react';
...
export interface SourcingPillarData {
  id: 'audits' | 'sourcing' | 'qc' | 'logistics';
  serviceCode: 'FACTORY_AUDIT' | 'PRODUCT_SOURCING' | 'QUALITY_CONTROL' | 'LOGISTICS';
  icon: LucideIcon;
  title: string;
  tag: string;
  updateBadge: string;
  ...
  comprehensiveSpecification: string[];
  updates2026: string[];
  deliverables: string[];
  standards: string[];
}

export function getSourcingPillars(lang: Locale): SourcingPillarData[] {
  const isZh = lang === 'zh';
  const isAr = lang === 'ar';
  ...
  return [
    // -------------------------------------------------------------
    // PILLAR 1: FACTORY AUDITS
    // -------------------------------------------------------------
    {
      id: 'audits',
      serviceCode: 'FACTORY_AUDIT',
      icon: Factory,
      title: isZh ? '原厂合规审计' : isAr ? 'تدقيق المصانع الميداني' : isCkb ? 'پشکنینی مەیدانی کارگەکان' : 'On-Site Factory Audits',
      tag: isZh ? '48小时现场进驻' : isAr ? 'فحص ميداني خلال 48 ساعة' : isCkb ? 'پشکنینی مەیدانی ٤٨ کاتژمێر' : '48h Field Dispatch',
      updateBadge: isZh ? '2026中伊双边验厂互认准则' : isAr ? 'بروتوكول التدقيق المعتمد 2026' : isCkb ? 'ستانداردی نوێکراوەی ٢٠٢٦' : '2026 Bilateral Audit Protocol',
      desc: isZh 
        ? '中伊通讯社专业工程与法务团队深入中国制造车间，实地审查资质、环保、真实产能及中东极端工况适配力。' 
        : isAr 
        ? 'زيارات تفتيش ميدانية لمصانع الصين للتحقق من الموثوقية القانونية والمالية والطاقة الإنتاجية الحقيقية.' 
        : isCkb 
        ? 'سەردانی مەیدانی بۆ کارگە چینییەکان بۆ دڵنیابوون لە شایستەیی یاسایی و توانای ڕاستەقینەی بەرهەمهێنان.' 
        : 'On-site technical evaluation of production lines, legal compliance, real capacity, and Middle East desert resilience.',
      leadSummary: isZh
        ? '依托常驻北京、上海及广州的工贸核验专员，杜绝空壳贸易中介与产能欺诈，出具具法律效力的25页权威实地验厂报告。'
        : isAr
        ? 'حماية المستورد العراقي من مخاطر الشركات الوهمية والوسطاء غير المعتمدين من خلال التدقيق الميداني الشامل في مواقع الإنتاج.'
        : isCkb
        ? 'پاراستنی سەرمایەی بازرگانانی عێراق و کوردستان لە ڕێگەی پشکنینی ڕاستەوخۆ لە ناو کارگەکانی چین بە ڕاپۆرتی فەرمی.'
        : 'Sovereign on-the-ground technical and corporate verification across Chinese manufacturing hubs to eliminate intermediary shell entities and guarantee industrial execution capacity.',
      wordCount: 335,
      readTime: isZh ? '2 分钟精读' : isAr ? 'قراءة في دقيقتين' : isCkb ? '٢ خولەک خوێندنەوە' : '2 Min Read',
      metrics: [
        {
          label: isZh ? '响应时效' : isAr ? 'سرعة الإيفاد' : isCkb ? 'کاتی گەیشتن' : 'Dispatch Speed',
          value: '48 - 72 Hours',
          sub: isZh ? '工程师现场进驻' : isAr ? 'وصول فريق التفتيش' : isCkb ? 'گەیشتن بۆ کارگە' : 'On-Site Arrival'
        },
        {
          label: isZh ? '管辖覆盖' : isAr ? 'النطاق الجغرافي' : isCkb ? 'ڕووبەری کار' : 'Jurisdiction',
          value: 'All 31 Provinces',
          sub: isZh ? '覆盖主要制造重镇' : isAr ? 'كافة المقاطعات الصينية' : isCkb ? 'سەرجەم پارێزگاکان' : 'Mainland China Hubs'
        },
        {
          label: isZh ? '官方背书' : isAr ? 'المرجعية الرقابية' : isCkb ? 'متمانەپێدان' : 'Accreditation',
          value: 'CCPIT & NECIPS',
          sub: isZh ? '国家企业信用档案' : isAr ? 'نظام الائتمان وسجل الشركات' : isCkb ? 'تۆماری فەرمی بازرگانی' : 'National Registry'
        },
        {
          label: isZh ? '欺诈防御' : isAr ? 'الحماية من الاحتيال' : isCkb ? 'پاراستنی سەرمایە' : 'Risk Protection',
          value: '100% Anti-Fraud',
          sub: isZh ? '零空壳公司承诺' : isAr ? 'منع الوسطاء الوهميين' : isCkb ? 'ڕێگری لە ساختەکاری' : 'Zero Shell Entities'
        }
      ],
      comprehensiveSpecification: isZh ? [
      ...
      updates2026: [
        isZh ? '2026年正式连通中国贸促会（CCPIT）与伊拉克贸易部双向涉企信用黑白名单数据库' : isAr ? 'الربط المباشر مع قاعدة بيانات المجلس الصيني (CCPIT) ودائرة تسجيل الشركات العراقية لعام 2026' : isCkb ? 'پەیوەندی ڕاستەوخۆ بە داتابەیسی باوەڕپێکراوی ٢٠٢٦' : 'Direct live data-exchange with CCPIT corporate registry and Iraqi Ministry of Trade Companies Directorate.',
        isZh ? '出台针对中东地区+55°C极端高温与沙漠沙尘环境的专属原厂耐受力实测认证准则' : isAr ? 'اعتماد ملحق فحص الصمود المناخي للشرق الأوسط (مقاومة درجات الحرارة فوق 50+ والغبار الصحراوي)' : isCkb ? 'پشکنینی تایبەت بۆ بەرگەگرتنی گەرمای ٥٥+ پلە' : 'Mandatory environmental resilience testing for extreme Middle East heat (+55°C) and desert dust ingress.',
        isZh ? '全线启用带防伪时间戳与GPS北斗双模卫星加密水印的4K实景勘查影像留痕系统' : isAr ? 'نظام التوثيق الفيديوي عالي الدقة (4K) المزود بالأختام الزمنية الفضائية المشفرة غير القابلة للتزييف' : isCkb ? 'سیستەمی ڤیدیۆیی 4K بە لۆکەیشنی GPS' : 'Tamper-evident 4K video inspection logs verified with dual Beidou/GPS cryptographic timestamps.'
      ],
      deliverables: [
        isZh ? '25页中阿/中英双语版《权威实地验厂综合评估报告书》（盖中伊通讯社官方防伪钢印）' : isAr ? 'ملف تدقيق رسمي ثنائي اللغة من 25 صفحة مختوم بختم الوكالة الرسمي' : isCkb ? 'ڕاپۆرتی ٢٥ لاپەڕەیی بە مۆری فەرمی' : '25-Page Signed & Stamped Bilingual Factory Audit Dossier',
        isZh ? '国家企业信用信息公示系统（NECIPS）官方征信穿透底档与司法诉讼审查报告' : isAr ? 'سجل الائتمان التجاري الصيني المعتمد والفحص القضائي الشامل' : isCkb ? 'تۆماری فەرمی باج و مۆڵەتی کارگە' : 'Full NECIPS Enterprise Registry & Judicial Litigation Extract',
        isZh ? '现场4K高清生产线、实验室仪器实测及仓储库容全景录像与高清特写图集' : isAr ? 'تسجيلات فيديو 4K للمصنع وصور فوتوغرافية عالية الدقة للمعدات' : isCkb ? 'ڤیدیۆی 4K و وێنەی سەرجەم بەشەکانی کارگە' : '4K Video Inspection Walkthrough & High-Resolution Photographic Log',
        isZh ? '中伊双边贸易委员会合规评级证书与采购风险对冲决策建议书' : isAr ? 'شهادة التقييم الائتماني النهائي وتوصية الشراء والتعاقد الآمن' : isCkb ? 'بڕوانامەی متمانەی کارگە و ڕێنمایی گرێبەست' : 'Commercial Risk Scoring Matrix & Legal Procurement Clearance Advisory'
      ],
      standards: ['ISO 9001:2015', 'CCPIT Verified', 'Iraqi MoT Protocol', 'NECIPS Audited']
    },

    // -------------------------------------------------------------
    // PILLAR 2: PRODUCT SOURCING
    // -------------------------------------------------------------
    {
      id: 'sourcing',
      serviceCode: 'PRODUCT_SOURCING',
      icon: Search,
      title: isZh ? '一手货源直采' : isAr ? 'توريد البضائع والآلات' : isCkb ? 'دابینکردنی ڕاستەوخۆ' : 'Direct Product Sourcing',
      tag: isZh ? '直通头部产业集群' : isAr ? 'ربط مباشر بالمجمعات الصناعية' : isCkb ? 'ناوەندە پیشەسازییەکان' : 'Tier-1 Industrial Clusters',
      updateBadge: isZh ? '2026出厂价闭环竞价走廊' : isAr ? 'ممر الشراء بسعر المصنع 2026' : isCkb ? 'ڕێڕەوی کڕین بە نرخی کارگە ٢٠٢٦' : '2026 Direct Wholesale Corridor',
      desc: isZh 
        ? '直接连通源头产业带与国家级产业集群，以一手出厂批发价采购重型工程设备、光伏储能与大宗工业原料。' 
        : isAr 
        ? 'وصول مباشر إلى كبرى المجمعات الصناعية الصينية لشراء الآلات ومعدات الطاقة والمواد الخام بأسعار المصنع.' 
        : isCkb 
        ? 'دەستڕاگەیشتنی ڕاستەوخۆ بە کارگە سەرەکییەکانی چین بۆ کڕینی ئامێری قورس و پێداویستییەکان بە نرخی کارگە.' 
        : 'Direct access to premier Chinese manufacturing clusters for industrial machinery, solar energy systems, and raw materials at verified wholesale rates.',
      leadSummary: isZh
        ? '剔除传统外贸中18%至35%的多层倒手加价，依托双边专席开展规范竞价，直通江苏常州、徐州、广东佛山等制造业腹地。'
        : isAr
        ? 'إلغاء هوامش الربح المتراكمة للوسطاء بنسبة 18% إلى 35% من خلال ربط المستوردين العراقيين بمصادر الإنتاج الأولية مباشرة.'
        : isCkb
        ? 'لابردنی دەڵاڵ و ناوەندگیرەکان و کەمکردنەوەی تێچوو بە ڕێژەی ١٨٪ تا ٣٥٪ بە پەیوەندی ڕاستەوخۆ لەگەڵ کارگەکان.'
        : 'Bypassing speculative middleman markups by connecting Iraqi procurement directly with China’s premier state-certified industrial clusters and Tier-1 original equipment manufacturers (OEMs).',
      wordCount: 335,
      readTime: isZh ? '2 分钟精读' : isAr ? 'قراءة في دقيقتين' : isCkb ? '٢ خولەک خوێندنەوە' : '2 Min Read',
      metrics: [
        {
          label: isZh ? '中间商降本' : isAr ? 'وفر التكلفة' : isCkb ? 'داشکاندنی تێچوو' : 'Cost Savings',
          value: '18% - 35%',
          sub: isZh ? '剔除中间倒手溢价' : isAr ? 'إلغاء عمولات الوسطاء' : isCkb ? 'لابردنی قازانجی دەڵاڵ' : 'Middleman Markup Cut'
        },
        {
          label: isZh ? '货源层级' : isAr ? 'فئة المصانع' : isCkb ? 'ئاستی کارگەکان' : 'Manufacturer Tier',
          value: 'Tier-1 & State Hubs',
          sub: isZh ? '中国机械联合会骨干' : isAr ? 'مصانع الفئة الأولى المعتمدة' : isCkb ? 'کارگە نایابەکان' : 'Original Equipment Mfrs'
        },
        {
          label: isZh ? '合同管辖' : isAr ? 'الصيغة القانونية' : isCkb ? 'گرێبەستی یاسایی' : 'Contract Law',
          value: 'Dual Jurisdiction',
          sub: isZh ? '中阿双语权威法律文本' : isAr ? 'صيغة قانونية ثنائية اللغة' : isCkb ? 'گرێبەستی عەرەبی و چینی' : 'Bilingual Enforceable'
        },
        {
          label: isZh ? '结算通道' : isAr ? 'قناة الدفع' : isCkb ? 'شێوازی پارەدان' : 'Payment Escrow',
          value: 'e-CNY / IQD Escrow',
          sub: isZh ? '支持主权数字人民币清算' : isAr ? 'حسابات ضمان باليوان الرقمي' : isCkb ? 'حسابی پارێزراوی دراو' : 'Sovereign Clearing Eligible'
        }
      ],
      comprehensiveSpecification: isZh ? [
      ...
      updates2026: [
        isZh ? '全面打通伊拉克中央银行（CBI）与数字人民币（e-CNY）定向工程采购托管履约账户' : isAr ? 'تفعيل حسابات الضمان المشتركة بين البنك المركزي العراقي وبنك الشعب الصيني (e-CNY) لعقود التوريد 2026' : isCkb ? 'چالاککردنی حسابی پارێزراوی هاوبەش بۆ دابینکردن بە یوان' : 'Direct escrow integration linking CBI institutional accounts and PBOC digital Yuan (e-CNY).',
        isZh ? '建立重型工程机械、光伏组件及电网变压器专项出厂成本透明对标模型' : isAr ? 'نموذج الشفافية السعرية المعتمد للآلات الثقيلة ومعدات الطاقة المتجددة ومحولات الطاقة' : isCkb ? 'مۆدێلی نرخی ڕاستەقینەی ئامێرە قورسەکان و وزەی خۆر' : 'Official transparent cost-benchmarking indexes for heavy machinery, solar PV, and power gear.',
        isZh ? '中阿双语标准采购合同中强制纳入5年原厂核心备件质保与工程师派驻条款' : isAr ? 'إلزامية بند توريد قطع الغيار الأصلية لمدة 5 سنوات وضمان إيفاد المهندسين في العقود الرسمية' : isCkb ? 'مەرجی فەرمی دابینکردنی پارچەی یەدەگ بۆ ٥ ساڵ لە گرێبەستدا' : 'Mandatory 5-year OEM spare parts supply covenants and on-site commissioning guarantees in contracts.'
      ],
      deliverables: [
        isZh ? '原厂出厂价（Ex-Factory Price）全透明竞价清单与BOM物料成本比对报告' : isAr ? 'جدول مقارنة الأسعار المباشرة من المصنع وتحليل تكلفة المواد (BOM)' : isCkb ? 'خشتەی بەراوردی نرخ لە کارگە و پێکهاتەی کاڵا' : 'Itemized Ex-Factory Wholesale Price Comparison & BOM Benchmark Dossier',
        isZh ? '受双边国际经贸仲裁委认可的中阿双语法定采购协议（包含违约赔偿惩罚条款）' : isAr ? 'عقد شراء ملزم قانونياً باللغتين العربية والصينية خاضع للتحكيم الدولي' : isCkb ? 'گرێبەستی فەرمی بە دوو زمان بە مەرجی قەرەبووکردنەوە' : 'Dual-Jurisdiction Bilingual Commercial Purchase Agreement & Arbitration Clauses',
        isZh ? '原厂出具的5年核心部件正品保证书与售后技术工程师派驻授权书' : isAr ? 'شهادة ضمان أصلية للقطع الأساسية وتفويض فني معتمد لخدمات الصيانة' : isCkb ? 'بڕوانامەی گەرەنتی پارچەی ئەسڵی و خزمەتگوزاری چاککردنەوە' : 'Manufacturer 5-Year Core Component Warranty & Field Engineering Mandate',
        isZh ? '双边数字人民币托管履约确认函（Milestone Escrow Clearance Letter）' : isAr ? 'خطاب اعتماد مصرفي للدفع عبر الضمان المالي لمراحل الإنتاج (Escrow)' : isCkb ? 'پەسەندکردنی پارەدانی قۆناغ بە قۆناغ لە ڕێگەی حسابی پارێزراو' : 'Milestone Escrow Authorization Letter for Sovereign IQD/e-CNY Settlement'
      ],
      standards: ['BOM Audited', 'China Machinery Fed', 'CPIA Solar Standard', 'Bilingual Contract']
    },
    ...
    // -------------------------------------------------------------
    {
      ...
      icon: ShieldCheck,
      title: isZh ? '出港前质检 (QC)' : isAr ? 'مراقبة الجودة والفحص' : isCkb ? 'کۆنترۆڵی جۆرایەتی پێش بارکردن' : 'Pre-Shipment Quality Control',
      tag: isZh ? 'COSQC与ISO严格标准' : isAr ? 'معايير التقييس والسيطرة النوعية' : isCkb ? 'ستانداردی COSQC و ISO' : 'COSQC & ISO 17020 Standard',
      updateBadge: isZh ? '2026中东耐极端沙尘与高温测试' : isAr ? 'اختبارات الصمود لبيئة العراق 2026' : isCkb ? 'پشکنینی بەرگەگرتنی گەرما و تۆز' : '2026 Extreme Climate Stress Testing',
      desc: isZh 
        ? '离港前多重盲测抽检与中东耐候性能强化测试，全程监管装箱封条，杜绝瑕疵品与假冒伪劣货物出海。' 
        : isAr 
        ? 'بروتوكولات فحص صارمة متعددة المراحل قبل شحن الحاويات للقضاء على أي عيوب تصنيعية أو عدم مطابقة للمواصفات.' 
        : isCkb 
        ? 'پشکنینی توند پێش بارکردن بەپێی ستانداردە نێودەوڵەتییەکان بۆ ڕێگریکردن لە هەر کەموکوڕییەک لە بەرهەمەکاندا.' 
        : 'Multi-point statistical sampling, harsh Middle East environmental simulation, and supervised container sealing before vessel boarding.',
      leadSummary: isZh
        ? '严格依据伊拉克中央标准化与质量控制局（COSQC）准则与国际ISO/IEC 17020规范，执行100%独立盲抽与耐+55°C老化测试。'
        : isAr
        ? 'فحص شامل متوافق بنسبة 100% مع معايير الجهاز المركزي للتقييس والسيطرة النوعية العراقي (COSQC) واختبارات الإجهاد الحراري.'
        : isCkb
        ? 'پشکنینی ورد بەپێی ستانداردەکانی دەزگای کۆنترۆڵی جۆری عێراق (COSQC) و بەرگەگرتنی گەرمای سەروو ٥٥ پلە.'
        : 'Executing rigorous ANSI/ASQ Z1.4 blind sampling and certified Iraqi COSQC compliance testing to ensure zero defective goods enter Iraqi ports of entry.',
      wordCount: 330,
      readTime: isZh ? '2 分钟精读' : isAr ? 'قراءة في دقيقتين' : isCkb ? '٢ خولەک خوێندنەوە' : '2 Min Read',
      metrics: [
        {
          label: isZh ? '抽样标准' : isAr ? 'معيار أخذ العينات' : isCkb ? 'شێوازی نموونە' : 'Sampling Model',
          value: '100% Blind ANSI/ASQ',
          sub: isZh ? '独立抽检绝无弄虚作假' : isAr ? 'أخذ عينات عشوائي أعمى' : isCkb ? 'نموونەی هەڕەمەکی سەربەخۆ' : 'Zero Vendor Bias'
        },
        {
          label: isZh ? '耐候模拟' : isAr ? 'اختبار المناخ' : isCkb ? 'تاقیکردنەوەی کەشوهەوا' : 'Climate Stress',
          value: '+55°C Desert Test',
          sub: isZh ? '耐极端高温与沙尘防护' : isAr ? 'مقاومة درجات الحرارة والغبار' : isCkb ? 'بەرگەگرتنی گەرما و تۆز' : 'IP65/IP68 Validated'
        },
        {
          label: isZh ? '通关凭证' : isAr ? 'شهادة التخليص' : isCkb ? 'بڕوانامەی دەرچوون' : 'Inspection Cert',
          value: 'Official COI Issued',
          sub: isZh ? '直传伊拉克港口海关' : isAr ? 'شهادة مطابقة رسمية معتمدة' : isCkb ? 'بڕوانامەی COI فەرمی' : 'Direct Port Acceptance'
        },
        {
          label: isZh ? '装箱防伪' : isAr ? 'حماية الحاويات' : isCkb ? 'مۆری کۆنتێنەر' : 'Tamper Sealing',
          value: 'QR Bolt Seals',
          sub: isZh ? '唯一加密防伪钢丝封条' : isAr ? 'أختام بولت فولاذية مشفرة' : isCkb ? 'مۆری پارێزراوی کۆددار' : 'Cryptographic Seals'
        }
      ],
      comprehensiveSpecification: isZh ? [
      ...
      updates2026: [
        isZh ? '全面执行2026中伊进出口检验检测互认大纲，直连伊拉克COSQC数字检验备案平台' : isAr ? 'التكامل الرقمي المباشر مع منصة الجهاز المركزي للتقييس والسيطرة النوعية العراقي (COSQC) لعام 2026' : isCkb ? 'پەیوەستبوونی دیجیتاڵی بە سیستەمی کۆنترۆڵی جۆری عێراق ٢٠٢٦' : 'Direct digital integration with Iraqi COSQC electronic conformity verification database.',
        isZh ? '针对中东电网谐波畸变、高压浪涌以及光伏逆变器高温降额设置专项实验室实测指标' : isAr ? 'إجراء اختبارات معملية إلزامية لمقاومة تذبذب الجهد الكهربائي وموجات الحرارة للأجهزة ومعدات الطاقة' : isCkb ? 'پشکنینی توند بۆ بەرگەگرتنی کارەبا و پلەی گەرمای بەرز' : 'Specialized laboratory test suites for power-grid voltage fluctuations and solar inverter thermal derating.',
        isZh ? '集装箱铅封升级为北斗/GPS感应与不可逆加密防伪二维码高保密钢丝锁' : isAr ? 'تطبيق أختام البولت الفولاذية الذكية المزودة برمز استجابة سريعة (QR) مشفر مع التتبع الفضائي' : isCkb ? 'مۆری زیرەکی فولازی بە کۆدی پارێزراوی QR' : 'Smart tamper-evident bolt seals equipped with cryptographic QR verification and GPS telemetry logs.'
      ],
      deliverables: [
        isZh ? '官方正本出港前批次质量检测证书（COI Certificate of Inspection，具海关免检绿色通道效力）' : isAr ? 'شهادة الفحص والتفتيش الرسمية (COI) المعتمدة للتخليص الجمركي الفوري' : isCkb ? 'بڕوانامەی پشکنینی کوالیتی (COI) بۆ دەرچوونی خێرای گومرگی' : 'Official Certificate of Inspection (CoI) Endorsed for Expedited Green-Lane Clearance',
        isZh ? '实验室耐高温（+55°C）、防尘（IP65/IP68）与抗浪涌耐压权威实验数据报告' : isAr ? 'تقرير الاختبارات المعملية للتحمل الحراري والعزل ضد الغبار والتذبذب الكهربائي' : isCkb ? 'ڕاپۆرتی تاقیگەیی بەرگەگرتنی گەرما و تۆز' : 'Laboratory Climate Stress, IP Dust Protection & Dielectric Endurance Test Report',
        isZh ? '装箱全程4K照片与高清视频监装归档卷宗（含箱体无损检查与防潮布设记录）' : isAr ? 'ملف التوثيق الفيديوي والصوري لعملية التعبئة والتثبيت داخل الحاوية' : isCkb ? 'دۆکیومێنتی ڤیدیۆ و وێنەی بارکردنی کاڵاکان لە ناو کۆنتێنەر' : 'Full Photographic & Video Container Stuffing Supervision & Lashing Dossier',
        isZh ? '唯一加密编号高保钢封防伪凭条（Tamper-Evident High Security Seal Certificate）' : isAr ? 'شهادة الختم الأمني الفولاذي المرقم للحاوية' : isCkb ? 'بڕوانامەی مۆری پارێزراوی کۆنتێنەر' : 'Numbered Cryptographic Bolt Seal Verification Certificate with QR Hash'
      ],
      standards: ['COSQC Aligned', 'ISO/IEC 17020', 'ANSI/ASQ Z1.4', 'IP68 Certified']
    },

    // -------------------------------------------------------------
    // PILLAR 4: LOGISTICS & CUSTOMS CLEARANCE
    // -------------------------------------------------------------
    {
      id: 'logistics',
      serviceCode: 'LOGISTICS',
      icon: Ship,
      title: isZh ? '航运与绿色通关' : isAr ? 'الشحن والتخليص الجمركي' : isCkb ? 'گواستنەوە و گومرگی خێرا' : 'Logistics & Customs Clearance',
      tag: isZh ? '乌姆盖斯尔港绿色走廊' : isAr ? 'ممر أم قصر الأخضر' : isCkb ? 'ڕێڕەوی خێرای ئوم قەسر' : 'Umm Qasr Green Corridor',
      updateBadge: isZh ? '2026免滞港费快速清关专班' : isAr ? 'إعفاء من غرامات التأخير 2026' : isCkb ? 'لێخۆشبوون لە غەرامەی دواکەوتن' : '2026 Zero-Demurrage Express Line',
      desc: isZh 
        ? '直通中国各大海港至伊拉克乌姆盖斯尔港与祖拜尔港专属集装箱航线，打通免滞期费绿色通关与保税内陆护送。' 
        : isAr 
        ? 'إدارة لوجستية متكاملة من موانئ الصين إلى ميناء أم قصر والبصرة مع فترة سماح ممتدة وتخليص جمركي معجل.' 
        : isCkb 
        ? 'گواستنەوەی دەریایی لە بەندەرەکانی چینەوە بۆ بەندەری ئوم قەسر بە کاتی بەخۆڕایی درێژخایەن و گومرگی خێرا.' 
        : 'Dedicated direct maritime freight from Ningbo/Shanghai to Umm Qasr Port featuring 21-28 free demurrage days and pre-arrival ASYCUDA customs clearance.',
      leadSummary: isZh
        ? '携手中远海运（COSCO）、中外运及伊拉克交通部港口总局，将海运周期压缩至21-24天，并提供长达28天的免滞箱期保障。'
        : isAr
        ? 'بالتعاون مع خطوط COSCO وسلطات الموانئ العراقية، تقليص مدة الشحن البحري إلى 21-24 يوماً مع 21 إلى 28 يوماً مجانية دون غرامات تأخير.'
        : isCkb
        ? 'بە هاوکاری لەگەڵ کۆمپانیای COSCO و بەندەرەکانی عێراق، گەیشتن لە ٢١-٢٤ ڕۆژدا بە ٢١ بۆ ٢٨ ڕۆژ کاتی بەخۆڕایی بەبێ غەرامە.'
        : 'Operating strategic maritime alliances with COSCO Shipping, Sinotrans, and General Company for Ports of Iraq to deliver 21-day express transit and zero-demurrage protection.',
      wordCount: 335,
      readTime: isZh ? '2 分钟精读' : isAr ? 'قراءة في دقيقتين' : isCkb ? '٢ خولەک خوێندنەوە' : '2 Min Read',
      metrics: [
        {
          label: isZh ? '航运时效' : isAr ? 'مدة الإبحار' : isCkb ? 'ماوەی گەیشتن' : 'Maritime Transit',
          value: '21 - 24 Days',
          sub: isZh ? '宁波/上海直达乌姆盖斯尔' : isAr ? 'من الصين إلى ميناء أم قصر' : isCkb ? 'لە چینەوە بۆ ئوم قەسر' : 'Direct Express Route'
        },
        {
          label: isZh ? '免滞期保障' : isAr ? 'فترة السماح' : isCkb ? 'کاتی بەخۆڕایی' : 'Demurrage Free',
          value: '21 - 28 Days',
          sub: isZh ? '彻底杜绝高额滞箱罚金' : isAr ? 'أيام مجانية دون غرامات تأخير' : isCkb ? 'بەبێ غەرامەی دواکەوتن' : 'Zero Terminal Penalties'
        },
        {
          label: isZh ? '数字清关' : isAr ? 'النظام الجمركي' : isCkb ? 'سیستەمی گومرگ' : 'Customs Platform',
          value: 'ASYCUDA World',
          sub: isZh ? '到港前电子预申报' : isAr ? 'تخليص إلكتروني مسبق معتمد' : isCkb ? 'پاککردنەوەی ئەلیکترۆنی' : 'Pre-Arrival Digital Entry'
        },
        {
          label: isZh ? '内陆押运' : isAr ? 'النقل الداخلي' : isCkb ? 'گواستنەوەی ناوخۆیی' : 'Inland Escort',
          value: 'IoT Telemetry Fleet',
          sub: isZh ? '直通巴格达及埃尔比勒' : isAr ? 'شاحنات مراقبة بالأقمار الصناعية' : isCkb ? 'بارهەڵگری چاودێریکراو' : 'Satellite Monitored'
        }
      ],
      comprehensiveSpecification: isZh ? [
      ...
      updates2026: [
        isZh ? '全面接入伊拉克海关2026全新ASYCUDA World云端自动化清关预审系统' : isAr ? 'الربط المباشر مع نظام الأسيكودا العالمي (ASYCUDA World) المحدث لعام 2026 للتخليص الجمركي المسبق' : isCkb ? 'بەستنەوە بە سیستەمی ئەلیکترۆنی ASYCUDAی نوێکراوەی ٢٠٢٦' : 'Full digital interface with Iraqi Customs ASYCUDA World automated pre-arrival processing system.',
        isZh ? '中远海运专属协议：乌姆盖斯尔港北港与南港享21至28天超长免箱堆存期（Demurrage-Free）' : isAr ? 'اتفاقية حصرية مع COSCO لمنح 21 إلى 28 يوماً مجانية دون غرامات تأخير في ميناء أم قصر' : isCkb ? 'ڕێککەوتنی تایبەت بە کاتی بەخۆڕایی ٢١ تا ٢٨ ڕۆژ لە بەندەری ئوم قەسر' : 'Exclusive COSCO agreement granting 21-28 free demurrage days across Umm Qasr North & South berths.',
        isZh ? '开通通往埃尔比勒及苏莱曼尼亚的直达保税内陆绿色走廊，一单到底海关监管' : isAr ? 'تدشين ممر الترانزيت البري الجمركي المباشر إلى أربيل والسليمانية بنظام البيان الواحد' : isCkb ? 'ڕێڕەوی ڕاستەوخۆی گومرگی بۆ هەولێر و سلێمانی بەبێ وەستان' : 'Launch of bonded transit corridors to Erbil & Sulaymaniyah under single-window customs seal.'
      ],
      deliverables: [
        isZh ? '中远海运正本清洁提单（Original Bill of Lading）及28天免箱期官方背书凭证' : isAr ? 'بوليصة الشحن البحرية الأصلية النظيفة مع توثيق فترة السماح المجانية 28 يوماً' : isCkb ? 'پەلی باری دەریایی فەرمی بە تۆماری ٢٨ ڕۆژ کاتی بەخۆڕایی' : 'Original Clean Bill of Lading (B/L) with Endorsed 28-Day Demurrage-Free Rider',
        isZh ? '经中国贸促会（CCPIT）与伊拉克驻华使领馆认证的双签原产地证明书（CO）' : isAr ? 'شهادة المنشأ الأصلية مصدقة من السفارة العراقية في بكين والمجلس الصيني (CCPIT)' : isCkb ? 'بڕوانامەی لەدایکبوونی کاڵا بە مۆری باڵیۆزخانە و وەزارەتی دەرەوە' : 'Bilateral Attested Certificate of Origin (CO) Certified by CCPIT & Iraqi Embassy',
        isZh ? 'ASYCUDA系统绿色通道电子通关放行绿条与关税完税发票' : isAr ? 'إذن الإفراج الجمركي الأخضر الإلكتروني وقسيمة سداد الرسوم الكمركية الرسمية' : isCkb ? 'مۆڵەتی دەرچوونی سەوزی گومرگی بە شێوەی ئەلیکترۆنی' : 'Electronic ASYCUDA Green-Lane Customs Release Permit & Duty Clearance Receipt',
        isZh ? '北斗双模物联网车队内陆押运全程温湿度/GPS轨迹数字化监控报告' : isAr ? 'تقرير التتبع الفضائي لرحلة الشاحنات الداخلية مع سجلات مستشعرات الصدمات والحرارة' : isCkb ? 'ڕاپۆرتی چاودێری مانگی دەستکرد بۆ ڕێگای بارهەڵگرەکان تا گەیشتن بە کۆگا' : 'Satellite IoT Transit Traceability Log with Vibration & Thermal Shock Telemetry'
      ],
      standards: ['ASYCUDA World', 'COSCO Direct', 'GCPI Priority Berth', 'CCPIT Legalized']
    }
  ];
}
