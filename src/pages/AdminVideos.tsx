...
export default function AdminVideos() {
  ...
  useEffect(() => {
    if (editingVideo) {
      setFormData({
        ...
        coverUrl: editingVideo.coverUrl || '',
        category: editingVideo.category || '',
        region: editingVideo.region || '',
        guestName: editingVideo.guestName || '',
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingVideo?.id ? `/api/videos/${editingVideo.id}` : '/api/videos';
  ...
  return (
    <div className="space-y-6 text-start">
      ...
      {isEditing ? (
        <div key={editingVideo?.id || 'new'} className="bg-white p-6 rounded-xl border border-neutral-200">
          <h3 className="text-lg font-medium mb-4">{editingVideo?.id ? 'Edit Video' : 'New Video'}</h3>
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input required type="text" value={formData.videoUrl || ''} onChange={e => setFormData({...formData, videoUrl: e.target.value})} className="w-full p-2 border rounded" />
              </div>
              <div>
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <button type="submit" className="px-4 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700">Save Video</button>
            </div>
          </form>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden">
          <div className="overflow-x-auto">
            ...
            <thead className="bg-neutral-50 text-neutral-600">
              <tr>
                <th className="p-4 font-medium text-start">Video</th>
            ...
            <tbody className="divide-y divide-neutral-100">
                ...
                <tr key={p.id} className="hover:bg-neutral-50">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <button onClick={() => { setEditingVideo(p); setIsEditing(true); }} className="p-1 text-blue-600 hover:bg-blue-50 rounded">
              ...
              {videos.length === 0 && (
                <tr><td colSpan={5} className="p-4 text-center text-neutral-500">No videos found.</td></tr>
              )}
            </tbody>
          </table>
          </div>
