...
import { ChineseProduct } from '../types';
import { 
  Globe2, Plus, Search, Edit3, Trash2, 
  CheckCircle2, X, Image as ImageIcon, Save, AlertCircle 
} from 'lucide-react';

export function AdminChineseProducts() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingProduct, setEditingTopic] = useState<ChineseProduct | null>(null);
  ...
  const { data: products = [], isLoading } = useQuery<ChineseProduct[]>({
    queryKey: ['admin-chinese-products'],
    queryFn: async () => {
      const res = await apiFetch('/api/admin/chinese-products');
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-chinese-products'] });
      queryClient.invalidateQueries({ queryKey: ['chinese-products'] });
      closeModal();
    }
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      const res = await apiFetch(`/api/admin/chinese-products/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  useEffect(() => {
    if (editingProduct) {
      setFormData({
        ...
        titleAr: editingProduct.titleAr || '',
        titleZh: editingProduct.titleZh || '',
        titleCkb: editingProduct.titleCkb || '',
        descriptionEn: editingProduct.descriptionEn || '',
        descriptionAr: editingProduct.descriptionAr || '',
        descriptionZh: editingProduct.descriptionZh || '',
        descriptionCkb: editingProduct.descriptionCkb || '',
  ...
  const handleEdit = (product: ChineseProduct) => {
    setEditingTopic(product);
    setIsModalOpen(true);
  };
  ...
  const filteredTopics = products.filter(t => 
    ...
    <>
      <div className="space-y-6 text-start">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          >
            <Plus size={16} /> New Chinese Product
          </button>
        </div>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-gray-400 font-medium text-xs">
                      No products found in observatory.
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <div>
                            <div className="font-bold text-ink-900 leading-tight">{product.titleEn}</div>
                            {product.link && <div className="text-xs text-gray-400 font-medium mt-0.5">{product.link}</div>}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-end">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button 
                            onClick={() => { if(window.confirm('Delete product?')) deleteMutation.mutate(product.id); }}
      ...
      {isModalOpen && (
        <div key={editingProduct?.id || 'new'} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-900/60 backdrop-blur-sm">
          <div className="bg-white border-2 border-brand-800 w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-xs shadow-2xl animate-in fade-in zoom-in duration-200 text-start">
            <div className="sticky top-0 bg-paper-50 p-6 border-b border-gray-100 flex justify-between items-center z-10">
              <div>
                <h2 className="text-xl font-bold font-black text-brand-800 uppercase tracking-widest flex items-center gap-3">
                  {editingProduct ? 'Edit Chinese Product' : 'New Chinese Product'}
                </h2>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-6 min-w-0">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <option value="EVS">EVs & Auto</option>
              ...
              <div className="space-y-4">
                <label className="text-xs font-black uppercase tracking-widest text-gray-500">Product Description (Trilingual)</label>
                <div className="grid grid-cols-1 gap-6">
                   <textarea 
                     placeholder="English Description"
                     value={formData.descriptionEn}
                     onChange={(e) => setFormData({...formData, descriptionEn: e.target.value})}
                     className="w-full px-4 py-3 border border-gray-200 rounded-xs text-sm min-h-[100px]"
                   />
                   <div className="grid grid-cols-2 gap-4">
                      ...
                      <textarea 
                        ...
                        value={formData.descriptionCkb}
                        onChange={(e) => setFormData({...formData, descriptionCkb: e.target.value})}
                        className="w-full px-4 py-3 border border-gray-200 rounded-xs text-sm min-h-[80px] font-bold"
                      />
                   </div>
                   <textarea 
              ...
              <div className="pt-6 border-t border-gray-100 flex justify-end gap-3">
                ...
                >
                  {createMutation.isPending || updateMutation.isPending ? 'Synchronizing...' : (
                    <>
                      <Save size={16} /> {editingProduct ? 'Update Records' : 'Publish Topic'}
                    </>
                  )}
                </button>
              </div>
