  ...
  return (
    <div className="flex flex-col w-full bg-paper-50 dark:bg-neutral-900 transition-colors">
      ...
      <div className="w-full bg-white dark:bg-neutral-900 text-ink-900 dark:text-white border-b border-gray-200 dark:border-neutral-800 flex overflow-hidden h-12 relative items-center shadow-xs">
        <div className="flex-shrink-0 font-serif font-black text-brand-800 dark:text-brand-400 pe-6 border-e border-gray-200 dark:border-neutral-800 uppercase tracking-[0.2em] text-xs sm:text-sm z-10 bg-white dark:bg-neutral-900 h-full flex items-center px-4">
            ...
            {[...articles.slice(0, 8), ...articles.slice(0, 8)].map((article, i) => {
              ...
              return (
                 <div key={`${article.id}-${i}`} className="h-12 flex items-center shrink-0 cursor-pointer group" onClick={() => setSelectedArticle(article)}>
                   <span className="text-xs text-brand-800 dark:text-brand-400 font-mono me-4 shrink-0 uppercase font-bold">{new Date(article.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', hour12: false})}</span>
                   <span className="text-xs sm:text-sm font-black uppercase tracking-wider text-ink-900 dark:text-neutral-100 group-hover:text-brand-800 dark:group-hover:text-brand-400 transition-colors truncate">{tr?.title}</span>
                 </div>
              )
            })}
          </div>
      ...
      <section className="w-full p-4 sm:p-6 bg-brand-800 text-white border-y-4 border-brand-800 relative overflow-hidden shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg sm:text-xl font-black uppercase tracking-widest text-white font-serif">
              {t('trending')}
            </h3>
            <span className="hidden sm:inline-block text-xs text-white font-mono ms-2 bg-white/10 px-2.5 py-0.5 rounded-full border border-white/20 font-bold backdrop-blur-sm">
            ...
            return (
              ...
              >
                ...
                <div className="relative w-full h-[155px] sm:h-[165px] bg-black/30 overflow-hidden border-b border-white/15">
                  {article.imageUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-black/30 backdrop-blur-xs text-white font-bold">
                      <span className="font-serif italic text-xs text-white/90">
                  ...
                  <div className="absolute top-2.5 start-2.5 bg-black/50 backdrop-blur-md text-white text-[10px] font-black uppercase px-2.5 py-1 rounded-sm shadow-xs tracking-wider border border-white/25">
                ...
                <div className="p-4 flex flex-col justify-between flex-grow space-y-3 bg-white/5 backdrop-blur-xs">
                  <h4 className="font-serif text-sm sm:text-base font-bold tracking-tight text-white group-hover:text-white/95 transition-colors line-clamp-2 leading-snug antialiased relative z-10">
                    {tr?.title}
                  </h4>
                  ...
                  {/* Footer */}
                  <div className="flex items-center justify-between text-xs font-mono font-bold text-white/85 pt-3 border-t border-white/15 relative z-10">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x rtl:divide-x-reverse divide-gray-200 dark:divide-neutral-800 bg-white dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-800 shadow-xs">
        ...
        <div className="flex items-center gap-2 mb-4 sm:mb-6 uppercase text-brand-800 dark:text-brand-400 font-mono text-xs sm:text-sm font-black tracking-widest border-b border-gray-100 dark:border-neutral-800 pb-2">
        ...
        {leadStory && (
          ...
          >
              ...
              <div className="w-full h-[260px] sm:h-[300px] md:h-[400px] overflow-hidden">
                {leadStory.imageUrl ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-gray-400">
                    <span className="font-serif italic">Image not available</span>
                  </div>
                )}
              </div>
              <div className="absolute top-4 start-4 bg-brand-800 text-white text-[10px] font-black uppercase px-3 py-1.5 rounded-sm shadow-sm">
                {lang === 'ar' ? 'تقرير خاص' : lang === 'ckb' ? 'ڕاپۆرتی تایبەت' : lang === 'zh' ? '特别报道' : 'Special Report'}
              </div>
            </div>
            
            <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl font-black leading-[1.1] mb-5 px-1 tracking-tight text-ink-900 dark:text-neutral-100 group-hover:text-brand-800 dark:group-hover:text-brand-400 transition-colors">
              {getTranslation(leadStory)?.title}
            </h2>
            
            <div className="flex items-center space-x-4 rtl:space-x-reverse text-xs font-mono font-bold uppercase mb-6 px-1 text-gray-500 dark:text-neutral-400 border-y border-gray-100 dark:border-neutral-800 py-3">
            ...
            <div className="space-y-4 px-1 text-base sm:text-lg leading-relaxed text-gray-700 dark:text-neutral-300">
              <p className="first-letter:text-6xl first-letter:font-black first-letter:font-serif first-letter:float-start first-letter:pe-3 first-letter:text-brand-800 dark:first-letter:text-brand-400">
              ...
              <div className="pt-6 mt-4 flex items-center justify-between text-xs font-mono font-bold text-brand-800 dark:text-brand-400 border-t border-gray-100 dark:border-neutral-800 group-hover:underline">
      ...
      <section className="lg:col-span-5 flex flex-col p-4 sm:p-6 md:p-8 space-y-8 bg-neutral-50 dark:bg-neutral-900/50">
        <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-lg shadow-sm p-6 flex-grow">
          ...
          <div className="space-y-5">
            <div className="border-s-4 border-brand-800 ps-4 bg-gray-50 dark:bg-neutral-800/80 p-3.5 rounded-e-md shadow-xs">
              <div className="text-[10px] font-bold text-gray-500 dark:text-neutral-400 uppercase tracking-widest mb-1">Maysan Province</div>
            ...
            <div className="border-s-4 border-gray-300 dark:border-neutral-600 ps-4 bg-white dark:bg-neutral-800/40 border border-y-gray-100 border-r-gray-100 dark:border-y-neutral-800 dark:border-r-neutral-800 p-3.5 rounded-e-md">
              <div className="text-[10px] font-bold text-gray-500 dark:text-neutral-400 uppercase tracking-widest mb-1">Anbar Refinery</div>
          ...
          <div className="mt-10">
            <h3 className="text-base sm:text-lg uppercase font-black border-b border-gray-200 dark:border-neutral-800 pb-3 mb-6 text-ink-900 dark:text-neutral-100 tracking-wider">
              {t('popular')}
            </h3>
            <ol className="space-y-5 list-decimal list-inside text-base text-ink-900 dark:text-neutral-200">
              {popularStories.map((article) => (
                <li 
                  key={`pop-${article.id}`} 
                  onClick={() => setSelectedArticle(article)}
                  className="font-serif font-bold leading-snug cursor-pointer hover:text-brand-800 dark:hover:text-brand-400 transition-colors border-b border-gray-100 dark:border-neutral-800/50 pb-3 last:border-0 last:pb-0"
                >
        ...
        <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 p-6 rounded-lg shadow-sm hover:border-brand-800/40 transition-all duration-300">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-brand-50 dark:bg-brand-950/80 text-brand-800 dark:text-brand-400 border border-brand-200 dark:border-brand-900/50 rounded-sm font-black uppercase text-xs tracking-widest mb-4">
          ...
          <h3 className="font-serif font-black text-xl mb-2.5 leading-snug text-ink-900 dark:text-white">
          ...
          <Link 
            ...
            className="inline-flex items-center justify-between w-full px-4 py-3 bg-brand-800 hover:bg-brand-900 text-white rounded-md text-xs font-black uppercase tracking-wider transition-all shadow-sm group"
          >
      ...
      {articles.filter(a => a.category?.slug === 'opinion').length > 0 && (
        <section className="w-full bg-white dark:bg-neutral-900 border-t-4 border-b-2 border-brand-800 p-4 sm:p-6 md:p-8 my-6 animate-fadeIn shadow-xs">
          <div className="flex justify-between items-baseline border-b border-brand-800/15 pb-3 mb-6">
            <h2 className="font-serif text-2xl md:text-3xl font-black tracking-tight text-ink-900 dark:text-neutral-100 uppercase flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 divide-y md:divide-y-0 md:divide-x rtl:divide-x-reverse divide-ink-900/10 dark:divide-neutral-800">
            {articles.filter(a => a.category?.slug === 'opinion').slice(0, 4).map((article, idx) => {
              ...
              const avatarBg = ['bg-brand-50 dark:bg-brand-900/30', 'bg-amber-50 dark:bg-amber-900/30', 'bg-emerald-50 dark:bg-emerald-900/30', 'bg-stone-100 dark:bg-neutral-800'][idx % 4];
              const textAccent = ['text-brand-800 dark:text-brand-400', 'text-amber-800 dark:text-amber-400', 'text-emerald-800 dark:text-emerald-400', 'text-ink-900 dark:text-neutral-200'][idx % 4];
              ...
              return (
                ...
                >
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="leading-tight">
                        <h4 className="text-xs font-black uppercase tracking-wider text-ink-900 dark:text-neutral-100 group-hover:text-brand-800 transition-colors">
                          {authorName}
                        </h4>
                        <span className="text-[9px] uppercase font-bold text-gray-500 dark:text-neutral-400 block tracking-wider">
                          {idx < 2 ? (lang === 'zh' ? '特约撰稿人' : 'Contributor') : (lang === 'zh' ? '智库学者' : 'Guest Analyst')}
                        </span>
                      </div>
                    </div>

                    <h3 className="font-serif font-bold text-base sm:text-lg leading-snug text-ink-900 dark:text-white group-hover:text-brand-800 dark:group-hover:text-brand-400 transition-colors line-clamp-3 italic pt-2 border-t border-brand-800/10">
                      “{tr?.title}”
                    </h3>
                  ...
                  <div className="mt-4 pt-3 border-t border-neutral-100 dark:border-neutral-800 flex items-center justify-between text-[11px] font-mono font-bold text-neutral-500 dark:text-neutral-400">
        ...
        <section className="w-full bg-white dark:bg-neutral-900 border-t-4 border-double border-brand-800 p-4 sm:p-6 md:p-8 space-y-6 shadow-xs my-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-neutral-200 dark:border-neutral-800 pb-4 gap-3">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-serif text-2xl sm:text-3xl font-bold text-ink-900 dark:text-white tracking-tight">
                {t('studies')}
              </h2>
            </div>
            <span className="text-xs font-mono font-bold uppercase tracking-wider bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 px-3 py-1 border border-neutral-200 dark:border-neutral-700 rounded-md">
                ...
                >
                  <div className="space-y-4">
                    {study.imageUrl && (
                      <div className="relative w-full aspect-video overflow-hidden border-b border-neutral-200 dark:border-neutral-700">
                        ...
                        <div className="absolute top-3 start-3 flex gap-1.5">
                          {study.isPrivate ? (
                            <span className="bg-amber-600 text-white text-[10px] font-mono font-bold uppercase tracking-wider px-2.5 py-1 rounded shadow-sm">
                              PREMIUM
                            </span>
                          ) : (
                            <span className="bg-emerald-700 text-white text-[10px] font-mono font-bold uppercase tracking-wider px-2.5 py-1 rounded shadow-sm">
                              OPEN ACCESS
                            </span>
                          )}
                        </div>
                    ...
                    <div className="p-5 space-y-3">
                      <div className="flex items-center gap-2 text-xs font-mono uppercase text-neutral-500 dark:text-neutral-400 font-bold tracking-wider">
                        <span>{study.author?.name || 'Research Desk'}</span>
                        <span>•</span>
                        <span>{new Date(study.createdAt).toLocaleDateString(dateLocale.code)}</span>
                      </div>
                      <h3 className="font-serif text-base sm:text-lg font-bold leading-snug group-hover:text-brand-800 dark:group-hover:text-brand-400 text-ink-900 dark:text-white transition-colors line-clamp-3">
                        {getStudyTitle(study)}
                      </h3>
                      <p className="text-xs sm:text-sm text-neutral-600 dark:text-neutral-300 line-clamp-3 leading-relaxed">
        ...
        {['ai', 'food-beverage', 'expo', 'business-statistics'].map((catSlug) => {
          ...
          return (
            <div key={catSlug} className="p-4 sm:p-6 flex flex-col space-y-4">
              <h3 className="text-sm sm:text-base uppercase font-black tracking-widest text-ink-900 dark:text-neutral-100 border-b-2 border-brand-800 pb-1 w-fit">
                {title}
              </h3>
              {sectionArticles.map((article, i) => (
                ...
                >
                  <h4 className="font-serif text-sm font-bold leading-tight group-hover:text-brand-800 text-ink-900 dark:text-neutral-200 transition-colors line-clamp-3">
                    {getTranslation(article)?.title}
                  </h4>
                  <p className="text-[10px] text-gray-500 dark:text-neutral-400 mt-1 uppercase font-bold opacity-70">
              ...
              return (
                ...
                >
                  ...
                  <div className="p-3 flex flex-col flex-grow">
                    <span className="text-[9px] font-mono uppercase tracking-wider text-gray-500 dark:text-neutral-400 mb-2">
                      {new Date(article.createdAt).toLocaleDateString(dateLocale.code)}
                    </span>
                    <h3 className="font-serif font-bold text-sm leading-snug text-ink-900 dark:text-neutral-100 group-hover:text-brand-800 transition-colors line-clamp-3">
                      {tr?.title}
                    </h3>
                  </div>
                </div>
