    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 text-start">
      <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          ...
          <h3 className="text-2xl font-serif font-black text-white leading-tight">Authorize trilingual information dispatches.</h3>
          <p className="text-neutral-500 text-sm font-mono max-w-2xl">
      ...
      {telexes.length === 0 ? (
        <div className="py-24 text-center border border-neutral-900 bg-neutral-950 rounded-xl font-mono text-[10px] uppercase tracking-[0.2em] text-neutral-600">
           -- NO ACTIVE DISPATCHES IN LEDGER --
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6">
          {telexes.map((tlx) => (
            <div key={tlx.id} className="bg-neutral-950 border border-neutral-900 rounded-xl overflow-hidden shadow-xl font-mono flex flex-col md:flex-row group">
              <div className="md:w-64 bg-neutral-900/50 p-6 border-r border-neutral-900 flex flex-col justify-between">
                <div className="space-y-4">
                  <div className="flex items-center gap-2 text-[#00FF00] text-[10px] font-black uppercase tracking-widest border-b border-[#00FF00]/10 pb-2">
                    <Activity size={12} className="animate-pulse" />
                    SIGNAL ACTIVE
                  </div>
                  <div className="space-y-1">
                    <p className="text-[#00FF00] text-[11px] font-black">REF: {tlx.telexRef}</p>
                    <p className="text-neutral-500 text-[9px] uppercase tracking-wider">{tlx.bureau} Node</p>
                  </div>
                </div>
                <div className="pt-8">
                  <div className={`inline-flex px-2 py-0.5 border text-[9px] font-black uppercase tracking-[0.2em] ${
              ...
              <div className="flex-1 p-6 space-y-6">
                <div className="flex flex-col md:flex-row justify-between gap-4">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-neutral-500 uppercase tracking-widest">Sovereign Origin</p>
                    <p className="text-sm font-bold text-white">{tlx.name} <span className="text-neutral-500 font-normal">({tlx.company})</span></p>
                    <p className="text-[10px] text-brand-500 uppercase">{tlx.email}</p>
                  </div>
                  <div className="text-right">
                     <p className="text-[10px] font-black text-neutral-500 uppercase tracking-widest">Transmit Time</p>
                     <p className="text-[10px] text-neutral-300 font-bold">{new Date(tlx.createdAt).toLocaleString()}</p>
                  </div>
                </div>
                ...
                <div className="pt-4 flex justify-between items-center">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[9px] font-black text-neutral-600 uppercase tracking-widest">Processed via Global Ledger</span>
                  </div>
                  <div className="flex gap-3">
                    <button 
                      onClick={() => statusMutation.mutate({ id: tlx.id, status: 'REPLIED' })}
                      className="bg-[#00FF00]/10 hover:bg-[#00FF00]/20 text-[#00FF00] border border-[#00FF00]/30 text-[9px] font-black px-4 py-1.5 rounded uppercase tracking-widest transition-all cursor-pointer flex items-center gap-2"
                    >
                      <Check size={12} />
                      Acknowledge
                    </button>
                    <button 
                      ...
                      className="text-neutral-500 hover:text-white border border-neutral-800 hover:border-neutral-700 text-[9px] font-black px-4 py-1.5 rounded uppercase tracking-widest transition-all cursor-pointer flex items-center gap-2"
                    >
                      <Archive size={12} />
                      Archive
                    </button>
